...
| **配置系统**      | 可设置路径、最大数量、恢复确认阈值、自动启动游戏、语言等选项；所有配置与备注持久化。                                  |
| **自动启动游戏**    | 恢复完成后可选自动执行 `steam://rungameid/3419290`。                          |
| **多语言支持**      | 支持中文和英文界面，可手动切换语言或自动检测系统语言。                          |
...
- **自动检测：** 程序首次启动时自动检测系统语言，中文系统默认使用简体中文，其他系统默认使用英文
- **手动切换：** 用户可在设置窗口中手动选择界面语言
- **实时切换：** 语言更改后立即生效，无需重启程序

### 支持语言

- **简体中文 (zh_CN)：** 完整的中文界面，包括所有按钮、菜单、提示信息
- **英文 (en_US)：** 完整的英文界面，适合海外用户

### 翻译文件结构

```
src/godforsaken_save_manager/i18n/
├── __init__.py
├── translator.py     # 翻译器核心
├── zh_CN.json       # 简体中文翻译
└── en_US.json       # 英文翻译
...
语言设置保存在 `backup_manager_config.json` 文件中：

```json
{
  "language": "zh_CN"  // null表示自动检测，"zh_CN"表示简体中文，"en_US"表示英文
}
```
...
提供：
* 支持点分隔的层级翻译键（如 `ui.main_window.title`）
* 自动系统语言检测
* 实时语言切换
* 参数化字符串格式化
* 翻译文件缺失时的优雅降级
...
翻译文件，采用JSON格式存储翻译键值对：

```json
{
  "ui": {
    "main_window": {
      "title": "神弃之地存档备份管理器 v1.0",
      "backup_button": "备份当前存档"
    }
  },
  "dialogs": {
    "confirm_delete": "确认删除"
...
| 类别   | 优点                                    |
| ---- | ------------------------------------- |
| 架构   | 分层清晰：`core`（业务）、`ui`（界面）、`common`（工具）、`i18n`（国际化） |
| UI   | 简洁：仅展示时间与备注，避免视觉噪音                    |
| 国际化 | 完整的多语言支持，自动检测系统语言，实时切换              |
| 安全性  | 自动备份保护机制 + 进程检测 + 时间差值确认提醒            |
| 配置   | 自动应用默认值 + 可编辑存储路径 + 可自定义恢复确认阈值 + 语言配置 |
| 编译   | 使用 Nuitka 实现高性能单文件可执行产物               |
| 可维护性 | 每个模块职责单一，可独立测试，翻译文件独立管理              |
